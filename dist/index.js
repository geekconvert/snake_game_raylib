...
// include: /var/folders/q7/65nlg0zd5hjgcqv94sy5d8740000gn/T/tmp60weayhn.js
...
Module['FS_createPath']("/", "Sounds", true, true);
    ...
    loadPackage({"files": [{"filename": "/Graphics/food.png", "start": 0, "end": 217}, {"filename": "/Sounds/eat.mp3", "start": 217, "end": 17881, "audio": 1}, {"filename": "/Sounds/wall.mp3", "start": 17881, "end": 65497, "audio": 1}], "remote_package_size": 65497});

  })();

// end include: /var/folders/q7/65nlg0zd5hjgcqv94sy5d8740000gn/T/tmp60weayhn.js
// include: /var/folders/q7/65nlg0zd5hjgcqv94sy5d8740000gn/T/tmpnidx1sfr.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if (Module['$ww'] || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /var/folders/q7/65nlg0zd5hjgcqv94sy5d8740000gn/T/tmpnidx1sfr.js
// include: /var/folders/q7/65nlg0zd5hjgcqv94sy5d8740000gn/T/tmplr6tzeos.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /var/folders/q7/65nlg0zd5hjgcqv94sy5d8740000gn/T/tmplr6tzeos.js
...
var ASM_CONSTS = {
  328744: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 330902: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 331066: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 331170: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 331341: ($0, $1, $2, $3, $4, $5) => { var deviceType = $0; var channels = $1; var sampleRate = $2; var bufferSize = $3; var pIntermediaryBuffer = $4; var pDevice = $5; if (typeof(window.miniaudio) === 'undefined') { return -1; } var device = {}; var audioContextOptions = {}; if (deviceType == window.miniaudio.device_type.playback && sampleRate != 0) { audioContextOptions.sampleRate = sampleRate; } device.webaudio = new (window.AudioContext || window.webkitAudioContext)(audioContextOptions); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; var channelCountIn = 0; var channelCountOut = channels; if (deviceType != window.miniaudio.device_type.playback) { channelCountIn = channels; } device.scriptNode = device.webaudio.createScriptProcessor(bufferSize, channelCountIn, channelCountOut); device.scriptNode.onaudioprocess = function(e) { if (device.intermediaryBufferView == null || device.intermediaryBufferView.length == 0) { device.intermediaryBufferView = new Float32Array(Module.HEAPF32.buffer, pIntermediaryBuffer, bufferSize * channels); } if (deviceType == miniaudio.device_type.capture || deviceType == miniaudio.device_type.duplex) { for (var iChannel = 0; iChannel < channels; iChannel += 1) { var inputBuffer = e.inputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { intermediaryBuffer[iFrame*channels + iChannel] = inputBuffer[iFrame]; } } _ma_device_process_pcm_frames_capture__webaudio(pDevice, bufferSize, pIntermediaryBuffer); } if (deviceType == miniaudio.device_type.playback || deviceType == miniaudio.device_type.duplex) { _ma_device_process_pcm_frames_playback__webaudio(pDevice, bufferSize, pIntermediaryBuffer); for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { var outputBuffer = e.outputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { outputBuffer[iFrame] = intermediaryBuffer[iFrame*channels + iChannel]; } } } else { for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { e.outputBuffer.getChannelData(iChannel).fill(0.0); } } }; if (deviceType == miniaudio.device_type.capture || deviceType == miniaudio.device_type.duplex) { navigator.mediaDevices.getUserMedia({audio:true, video:false}) .then(function(stream) { device.streamNode = device.webaudio.createMediaStreamSource(stream); device.streamNode.connect(device.scriptNode); device.scriptNode.connect(device.webaudio.destination); }) .catch(function(error) { console.log("Failed to get user media: " + error); }); } if (deviceType == miniaudio.device_type.playback) { device.scriptNode.connect(device.webaudio.destination); } device.pDevice = pDevice; return miniaudio.track_device(device); },  
 334169: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 334235: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 334628: ($0) => { miniaudio.untrack_device_by_index($0); },  
 334671: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 334796: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; },  
 334922: () => { if (document.fullscreenElement) return 1; },  
 334968: () => { return Module.canvas.width; },  
 335000: () => { return parseInt(Module.canvas.style.width); },  
 335048: () => { document.exitFullscreen(); },  
 335075: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 335148: () => { if (document.fullscreenElement) return 1; },  
 335194: () => { return Module.canvas.width; },  
 335226: () => { return screen.width; },  
 335251: () => { document.exitFullscreen(); },  
 335278: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 335411: () => { return window.innerWidth; },  
 335437: () => { return window.innerHeight; },  
 335464: () => { if (document.fullscreenElement) return 1; },  
 335510: () => { return Module.canvas.width; },  
 335542: () => { return parseInt(Module.canvas.style.width); },  
 335590: () => { if (document.fullscreenElement) return 1; },  
 335636: () => { return Module.canvas.width; },  
 335668: () => { return screen.width; },  
 335693: () => { return window.innerWidth; },  
 335719: () => { return window.innerHeight; },  
 335746: () => { if (document.fullscreenElement) return 1; },  
 335792: () => { return Module.canvas.width; },  
 335824: () => { return screen.width; },  
 335849: () => { document.exitFullscreen(); },  
 335876: () => { if (document.fullscreenElement) return 1; },  
 335922: () => { return Module.canvas.width; },  
 335954: () => { return parseInt(Module.canvas.style.width); },  
 336002: () => { document.exitFullscreen(); },  
 336029: ($0) => { Module.canvas.style.opacity = $0; },  
 336067: () => { return screen.width; },  
 336092: () => { return screen.height; },  
 336118: () => { return window.screenX; },  
 336145: () => { return window.screenY; },  
 336172: () => { return window.devicePixelRatio; },  
 336208: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 336261: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 336312: () => { Module.canvas.style.cursor = 'none'; },  
 336349: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 336605: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 336656: () => { if (document.fullscreenElement) return 1; },  
 336702: () => { return window.innerWidth; },  
 336728: () => { return window.innerHeight; },  
 336755: () => { if (document.pointerLockElement) return 1; }
};
